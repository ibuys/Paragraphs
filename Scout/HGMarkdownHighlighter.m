...
//                NSLog(@"%@", linkAddress);
                ...
                if (style.elementType == pmh_IMAGE)
                {
                    NSLog(@"pmh_IMAGE");
                    NSString *fullText = [self.targetTextView string];
                    NSString *original = [fullText substringWithRange:hlRange];
                    NSRange startRange = [original rangeOfString:@"["];
                    
                    NSRange endRange = [original rangeOfString:@"]"];
                    
                    NSRange range = NSMakeRange((startRange.location+[@"[" length]), (endRange.location-startRange.location-[@"[" length]));
                    
                    if (range.location != NSNotFound)
                    {
                        hlRange = NSMakeRange(hlRange.location + range.length + 3, hlRange.length - range.length - 3);
                    }
                    ...
                    linkAddress = [path stringByAppendingString:linkAddress];
                    [attrStr addAttribute:NSLinkAttributeName
                                    value:linkAddress
                                    range:hlRange];
